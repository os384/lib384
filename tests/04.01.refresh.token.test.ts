...
// NOTE on the 04.** tests:
//
// the test-setup flow is roughly that you would use the output from this test
// (eg 04.01), grab the console output and plug it into env.js, and other channel
// tests will use that to bootstrap from.
//
// that's why this test doesn't throw an exception if the token refresh fails,
